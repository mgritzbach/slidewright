# Slidewright 0.4.0 manual acceptance test
...
- Outer margins, repeated component padding, and limiting-line offsets are symmetric.
...
## Scenario 2 — source-native reference-deck reuse

Attach a presentation with at least 20 visually distinct slides that you own or are authorized to reuse. Ask for a new 8–12 slide deck on unrelated content using its design system and composition ideas.

Check:

- Recognizable reference compositions appear throughout the new deck, not only its colors.
- Supported masters, layouts, palette, gradients, guides, logos, limiting lines, recurring chrome, and available font tokens are inherited through native relationships.
- Any unavailable font or unsupported source construct is reported explicitly; no silent substitution is accepted.
- Open `design-provenance.json`: each substantive slide identifies its source, concept, composition variant, adaptation, confidence, item topology, and source-object bindings.
- The output is still a new presentation; source text is not copied accidentally.
- Save, close, and reopen the new deck in PowerPoint and confirm the source-native relationships remain intact.
...
2. Bold and unbold individual words in a mixed-emphasis paragraph. In a repeated `Label — explanation` list, every label must remain emphasized, every explanation regular, and a label-specific italic nuance must not spread into its body.
...
- Triangles keep center text inside their protected usable area.
- Squares and higher polygons remain regular rather than stretching to fill a rectangle.
- Labels sit adjacent to their shapes whenever possible; any necessary connector stays beneath nodes and text and matches the target rim.
- One emphasized aspect remains clearly unique without changing the geometry or typography of its peers.
...
Source-native inheritance retained:
