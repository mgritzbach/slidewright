...
3. If following an existing deck, inspect the complete source and preserve its typography, palette, spacing, placeholders, footers, guides, logos, master/layout binding, chrome, and reusable composition ideas. Read [references/design-profile.md](references/design-profile.md), [references/template-fidelity.md](references/template-fidelity.md), [references/structural-ingestion.md](references/structural-ingestion.md), and [references/visual-fidelity.md](references/visual-fidelity.md). Always run `scripts/slidewright.mjs profile <source.pptx> --out <profile.json>` across the complete deck. The profile must include one semantic concept record for every viable slide, normalized standard gradients, and non-blocking fidelity warnings. For an authorized source-native new sequence, use the rights-declared `compose-source-archetypes` content mode and `scripts/slidewright.mjs compose-profile`; do not claim completion before its structural, destructive, mapped-render, PowerPoint, and hash-bound full-size review gates pass. Use the structural importer only when a native PPTX must cross the workflow boundary without losing its slide/layout/master/theme hierarchy, text runs, tables, charts, diagrams, notes, or reading order. For a semantically grounded new deck that is not source-native composition, pass the profile to the guarded request with `--reference-profile <profile.json>` and require `design-provenance.json`. Structural ingestion is lossless preservation, not permission for arbitrary edits to imported objects. If the only source is an image, follow [references/image-ingestion.md](references/image-ingestion.md): emit a vision-derived observation record, label font identity as a guess, and keep source geometry isolated from rendering and scoring.
...
5. For every prompt-originated release build, read [references/prompt-robustness.md](references/prompt-robustness.md) and [references/copy-resilience.md](references/copy-resilience.md), preserve the exact original prompt in a strict request envelope, and run `scripts/slidewright.mjs request <request.json> --out <guarded-run> [--reference-profile <profile.json>]`. This immutable entry point owns policy, adaptive copy relayout, optional reference-concept matching and provenance, compile, font audit, plan lint, render, realized-layout lint, plan-bound OOXML audit, optional executive review, delivery verification, and atomic publication. Then run `scripts/slidewright.mjs request-verify <guarded-run> --out <verification.json>`. Conflicting prompts must reject before a plan or deck is published. Use the individual commands below only for diagnostics or non-release development; they do not qualify a prompt-originated delivery.
...
- For 2-9 parallel consulting points, use the count-aware `point-grid` grammar. Use `polygon-cycle` for 3-12 points only when they form a real cycle, system, perimeter, or mutually reinforcing model; its vertices must share one circumcircle, the center statement must occupy the visual centroid, and adjacent callouts replace leader lines. Use `quadrant-focus` for four domains around one outcome, `chevron-flow` for 3-5 true handoffs, and `icon-network` for honeycomb, pyramid, or square connected systems. Use `opposition` only for genuinely conflicting positions and add its synthesis band when the recommendation reconciles them. See [references/deck-spec.md](references/deck-spec.md).
- Prefer adjacency and intrinsic directional shapes over connectors. If a connector is necessary, render it beneath every node and text object, bind exact endpoints, and match the destination rim color; never let a line terminate visibly over a box or label.
