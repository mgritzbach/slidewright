...
Use `layout: "polygon-cycle"` only when 3-12 points form a genuine `cycle`, `system`, `perimeter`, or `mutual-reinforcement` relationship. The count determines a native editable segmented triangle through dodecagon; it does not justify the topology by itself. Every vertex lies on one true circumcircle with equal side lengths and apothems, every side is an independent trapezoidal beam with an upright editable marker, and its matched annotation module sits adjacent without a leader line. Each item requires `id`, `label`, and `body`; optional `marker` replaces its two-digit segment number, and optional `center` names the shared outcome or system. Four through twelve sides use the centroid. A triangle uses the largest axis-aligned text rectangle inside an inward-offset safe triangle, centered on that usable-space region so neither its backing nor its native text can touch a beam. At most one point may set `emphasis: true`, which highlights the beam and its bound annotation without changing peer geometry or typography.
...
Use `layout: "icon-network"` with `topology: "honeycomb"` and exactly seven items, `"pyramid"` with 3/6/10 items, or `"square"` with 4/9 items. Every node stays editable and may carry one icon, short label, and short body; at most one node may set `emphasis: true`. Honeycomb uses adjacency without connectors. Pyramid and square connectors render before every node and child label, route center-to-center so their endpoints are covered, bind exact endpoints, and match the destination node rim color and weight. `SW035` rejects invalid counts, geometry drift, visible/late connectors, route or endpoint drift, or rim-format mismatch.

The compiler also emits a `single-peer` emphasis contract for a declared focused point, polygon segment/callout, flow step, or network node. `quadrant-focus` emits a `synthesis` emphasis contract for its central diamond. `SW036` requires exactly one accent-coded focus and preserves the peer geometry; use no focus when the argument has no true priority or synthesis.
