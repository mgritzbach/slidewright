...
function Close-CapturedOwnedPresentation($Application, $CapturedPresentation, [string[]]$OwnedPresentationPaths, [string]$Context) {
  ...
  $capturedRawPath = [string]$CapturedPresentation.FullName
  $capturedPath = Get-NormalizedPresentationPath $capturedRawPath $allowedPaths
  if (-not $capturedPath -or -not ($allowedPaths -contains $capturedPath)) {
    throw "$Context close refused because the captured presentation path is not allowlisted. Raw='$capturedRawPath'; captured='$capturedPath'; allowed='$($allowedPaths -join ';')'."
  }
...
try {
  ...
  $primaryError = "Semantic round-trip failed during stage '$stage': $($_.Exception.Message)"
} finally {
  $cleanupError = $null
  if ($presentation) {
    try { Close-CapturedOwnedPresentation $powerPoint $presentation $ownedPaths "Semantic round-trip final cleanup after stage '$stage'" } catch { $cleanupError = $_.Exception.Message }
    $presentation = $null
  }
  if ($powerPoint) {
